...
export default function Navbar() {
  ...
  return (
    <>
      <div className="corporate-band text-white">
        <div className="container-pad flex min-h-11 flex-wrap items-center justify-between gap-2 py-2 text-[11px] font-semibold uppercase tracking-[0.18em]">
          <span>Corporate medical supply and lab procurement</span>
          <span className="text-white/70">Nairobi | +254 700 000 000 | info@medithrex.com</span>
        </div>
      </div>
      ...
      <header
        className={clsx(
          ...
          scrolled
            ? 'border-b border-white/70 bg-white/86 shadow-[0_28px_80px_-48px_rgba(8,59,116,0.5)] backdrop-blur-xl'
            : 'border-b border-slate-200/70 bg-white/95'
        )}
      >
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    clsx(
                      ...
                      isActive ? 'bg-blue-50 text-primary-700' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-950'
                    )
                  }
                >
                  {link.label}
            ...
            <div className="flex items-center gap-2">
              ...
              {isAuthenticated ? (
                <div className="relative hidden sm:block">
                  ...
                  {userMenu && (
                    <div className="absolute right-0 mt-3 w-56 overflow-hidden rounded-[1.6rem] border border-slate-200 bg-white shadow-[0_30px_80px_-38px_rgba(8,59,116,0.42)]">
                      <div className="border-b border-slate-100 px-5 py-4">
                        <div className="text-sm font-semibold text-slate-950">{user?.name}</div>
                        <div className="mt-1 text-xs text-slate-500">{user?.email}</div>
                      </div>
                      <Link to="/dashboard" onClick={() => setUserMenu(false)} className="flex items-center gap-3 px-5 py-3 text-sm text-slate-600 transition hover:bg-slate-50 hover:text-primary-700">
                        <Package className="h-4 w-4" />
                        My Orders
                      </Link>
                      {isAdmin() && (
                        <Link to="/admin" onClick={() => setUserMenu(false)} className="flex items-center gap-3 px-5 py-3 text-sm text-slate-600 transition hover:bg-slate-50 hover:text-primary-700">
                ...
                <div className="hidden items-center gap-2 sm:flex">
                  ...
                  <Link to="/register" className="btn-primary px-5 py-3 text-sm">Create Account</Link>
                </div>
              )}
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    clsx(
                      ...
                      isActive ? 'bg-blue-50 text-primary-700' : 'text-slate-700 hover:bg-slate-50'
                    )
                  }
                >
                  {link.label}
      ...
      {searchOpen && (
        <div className="fixed inset-0 z-[100] flex items-start justify-center bg-slate-950/40 px-4 pt-24 backdrop-blur-sm" onClick={() => setSearchOpen(false)}>
          <div className="w-full max-w-2xl overflow-hidden rounded-[1.8rem] border border-white/60 bg-white shadow-[0_40px_120px_-42px_rgba(8,59,116,0.5)]" onClick={(event) => event.stopPropagation()}>
            <form onSubmit={handleSearch} className="flex items-center gap-3 p-5">
              <div className="flex h-11 w-11 items-center justify-center rounded-full bg-blue-50 text-primary-700">
                <Search className="h-5 w-5" />
              </div>
              <input
                autoFocus
                ...
                placeholder="Search analyzers, monitors, consumables, lab equipment..."
